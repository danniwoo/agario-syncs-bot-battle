# v2.6 builds on top of my_bot_v2_5.py (Stage 1 - 3f). New in this file:
#   Stage 3g - 獵殺目標加一個下限，太小的獵物直接不追。
#              _can_eat_blob (engine/state/state_mutator.py) 除了質量門檻，
#              還要求「獵物中心點落在我方半徑內」。地圖邊界會把每顆 blob 的
#              中心點夾在 [radius, size-radius] 內，也就是說獵物退到真正的
#              角落時，中心點最多貼到 (prey_radius, prey_radius)；但我方體型
#              更大，中心點最多只能貼到 (my_radius, my_radius) -- 貼不了那麼
#              近。體型差太多時，就算雙方都貼到各自的極限，中心距離還是會
#              大於我方半徑，永遠吃不到（親眼在 replay 看到大體型卡在角落
#              前面吃不到小很多的對手，一直卡住）。算出最極端角落情境下還吃
#              得到的最小獵物半徑比例是 1 - 1/sqrt(2)，換算成質量比大約
#              8.6%，比這個更小的獵物就算追到天荒地老也吃不到，直接排除。
import math
...
# of how long we chase it -- see the Stage 3g note above for the derivation.
...
def choose_direction(game: Game, tracker: HuntTracker) -> tuple[float, float]:
    ...
    # Priority 2 [Stage 2 + Stage 3 + Stage 3c + 3e + 3g]: chase a locked-on,
    # safe-to-eat prey (using our strongest blob's mass, since only that blob
    # can actually land the kill) that's also big enough to be catchable even
    # in the worst case of it reaching a corner, giving up on chases that
    # aren't closing.
    prey_blobs = [
        blob
        for blob in enemies
