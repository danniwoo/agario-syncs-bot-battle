# v8 builds directly on my_bot_v3.py -- NOT on v6/v7. Mixed-field testing
# (one copy of each version + filler bots, the format that actually
# resembles the leaderboard) showed v6/v7's sibling-war wins didn't
# transfer: v3 ~= v6 >> v7 in a mixed field, matching the live leaderboard
# where v3 scored best. Lesson recorded: 4v4 same-lineage matches measure
# "who exploits their sibling", so all adoption decisions from here on use
# the mixed-field format with strong fillers.
#
...
#   Stage 7c - virus farming: VIRUS_FEED_MIN_MASS 16 -> 6. The exploitable-
#     pieces gate is the real safety mechanism; a high floor just refuses
#     free mass when alone. The scatter->flee->re-merge->next-virus farm
#     cycle emerges from the existing weakest-blob flee logic.
#
# --- Inherited from v3 ---
...
# --- Stage 4b: virus feeding / Stage 7c: virus farming ---
...
# Stage 7c drops the floor from 16 to 6: the real safety mechanism is the
# "no visible enemy can eat the scatter pieces" gate in _find_feedable_virus,
# which at small sizes only opens when we're alone anyway -- a high floor
# just refuses free mass in exactly the situations it's safe to take. The
# post-scatter danger is already handled by the weakest-blob flee logic
# (scattered pieces make everything look like a threat, so we run and
# re-merge, then come back for the next virus -- the farm cycle).
