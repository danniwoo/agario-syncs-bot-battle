# v9 = v6 + the three Stage 7 fixes from v8 (adjacent bite, no give-up when
# almost caught, virus farming floor 16 -> 6). Motivation: across both
# mixed-field evaluations (the format that resembles the leaderboard), v6
# consistently ranked at/near the top, and v8's fixes are bug repairs that
# should transfer to any lineage -- this combines the two best lines.
#
# --- Inherited from v6 ---
...
# Stage 7c (ported from v8): floor dropped 16 -> 6; the exploitable-pieces
# gate is the real safety mechanism, a high floor just refuses free mass
# in exactly the situations it's safe to take.
VIRUS_FEED_MIN_MASS = 6.0

# --- Stage 7a (ported from v8): opportunistic adjacent bite ---
# A clearly-eatable enemy within bite range is free mass right now -- no
# ratio filter, no tracker, no cooldown (HUNT_MASS_RATIO refused anything
# above 0.7x even at touching distance, and the give-up cooldown could fire
# against a cornered target juking along the wall).
ADJACENT_BITE_RANGE_FACTOR = 2.0
ADJACENT_BITE_MASS_RATIO = 1.25

# --- Stage 7b (ported from v8): never give up an almost-finished chase ---
GIVE_UP_MIN_DISTANCE_FACTOR = 2.0
...
def _select_prey(tracker: HuntTracker, prey_blobs, me, round_: int):
    ...
    if tracker.target_id is not None and tracker.target_id in by_id:
        ...
        if round_ - tracker.lock_round >= GIVE_UP_WINDOW_ROUNDS:
            # Stage 7b: a target this close is essentially caught -- the
            # stalled-distance give-up was firing right before the kill
            # against cornered prey juking along the wall.
            if distance >= tracker.lock_distance and distance > me.radius * GIVE_UP_MIN_DISTANCE_FACTOR:
                tracker.give_up(tracker.target_id, round_)
                return None
            tracker.lock_distance = min(distance, tracker.lock_distance)
...
def choose_direction(game: Game, tracker: HuntTracker) -> tuple[float, float, bool]:
    ...
    # Priority 1.5 [Stage 7a, ported from v8]: a clearly-eatable enemy within
    # bite range is free mass RIGHT NOW -- no ratio filter, no tracker lock,
    # no cooldown. (Still respects the corner-deadlock geometry so this can't
    # reintroduce the stuck-at-corner bug.)
    bite_range = me.radius * ADJACENT_BITE_RANGE_FACTOR
    bite_candidates = [
        blob
        for blob in enemies
        if strongest_mass > _mass(blob.radius) * ADJACENT_BITE_MASS_RATIO
        and math.hypot(blob.pos[0] - me.x, blob.pos[1] - me.y) <= bite_range
    ...
    if bite_candidates:
        ...
        target_x, target_y = _predicted_prey_position(me.x, me.y, bite, size)
        return (target_x - me.x, target_y - me.y, False)
