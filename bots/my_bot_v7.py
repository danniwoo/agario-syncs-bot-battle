# v7 builds on my_bot_v6.py:
#
#   Stage 6b - parameter tuning by head-to-head A/B (5 headless runs each):
#     LATE_GAME_MASS 12 -> 8 (the 8-variant beat 12 5/5 with ~7x the average
#     mass -- entering aggressive hunting earlier compounds hard; 5 vs 8 was
#     a wash so 8 stands). VIRUS_FEED_MIN_MASS stays 16 (the 12-variant lost:
#     smaller scatter pieces get punished).
#   Stage 6a - velocity-led split aim. Splits were observed visibly missing
#     laterally-moving prey: the aim came from _predicted_prey_position,
#     which only guesses "it flees straight away from us". A split is a
#     one-shot commitment (the ejected piece can't steer), so when firing
#     one, lead the shot using the target's MEASURED velocity (position
#     delta since last tick, tracked in HuntTracker.last_enemy_positions)
#     extrapolated over the estimated glide time. Normal walking chases keep
#     the naive prediction -- they re-aim every tick, so a lead there is the
#     already-rejected flanking experiment (v2, lost head-to-head).
#
# --- Inherited from v6 ---
# v6 builds on my_bot_v3.py (numbered v6 to avoid clashing with the separate
# v4/v5 lineage in this folder). Adds phase-aware tactics on top of v3:
#
#   Stage 5a - 前期 early game: virus bait. While we're small enough to be
#     virus-immune (strongest mass <= 2.7), bend the flee path over a virus
#     lying roughly along the escape heading; a bigger pursuer either breaks
#     off or gets scattered by it.
#   Stage 5b - 中期 mid game: split-threat-aware fleeing. An enemy whose
#     half-mass can still eat our weakest blob threatens us from its lunge
#     reach (~1.41*r + 8.9), not its walking range -- weight such threats up
#     while we're inside that reach so the safety buffer matches the burst.
#   Stage 5c - 後期 late game (strongest mass >= 12): hunt closer to the eat
#     limit (0.7 -> 0.78; hard cap is 1/1.2 ~= 0.83) and commit to longer
#     chases (30 -> 45 rounds). Food can't sustain a big blob, and a map
#     where nobody else is big is the safest place to be.
#   Stage 5d - fragment sniping prefers fragments with more merge_cooldown
#     left (a visible field on every blob), since they stay snipeable longer.
#
# Deliberately NOT added: true split-to-escape (isolation test E earlier
# showed no clear win in 5 runs, and halving piece mass makes us eatable by
# more enemies -- the Stage 5b buffer attacks the same death mode more
# safely).
#
# --- Inherited from v3 ---
# v3 attacks the late-game mass ceiling head-on. The diagnosis (all three
# are math, not vibes):
#
...
# --- Stage 5a: early-game virus bait ---
# While our strongest blob's mass is at or below virus_mass * EAT_SIZE_RATIO
# (2.7 for the standard 1.5-radius virus) a virus cannot split us, but it CAN
# split any pursuer above that mass -- and the engine's collision test (virus
# centre inside the blob's radius) means a big chaser tracing our path over a
# virus edge triggers it far more easily than we do. When fleeing during this
# immune window, bend the escape path toward a virus roughly along our flee
# heading: the pursuer either breaks off or gets scattered.
VIRUS_BAIT_MAX_DISTANCE = 12.0
VIRUS_BAIT_BLEND = 1.5

# --- Stage 5b: split-threat-aware fleeing ---
# An enemy whose HALF mass can still eat our weakest blob can kill us from
# ~sqrt(2)*its_radius + ~8.9 (eject glide) away in one burst via split-lunge
# -- the exact geometry our own Stage 4a exploits. Within that reach, weight
# its repulsion up so we keep a buffer sized for the burst, not the walk.
SPLIT_THREAT_WEIGHT_MULT = 3.0
SPLIT_EJECT_GLIDE = 8.9  # 1.6 / (1 - 0.82)

# --- Stage 5c: late-game aggression ---
# Once we're big, food can't sustain us (Stage 4b note) and the safest world
# is one where nobody else is big either: allow prey closer to the eat limit
# (1/1.2 ~= 0.83) and commit to longer chases.
# Stage 6b: tuned by A/B (5 headless runs each). 8 crushed 12 head-to-head
# (avg 13.3 vs 2.0, 5/5) -- earlier aggression compounds; 5 vs 8 was a
# wash, so 8 stands. VIRUS_FEED_MIN_MASS 12 lost to 16 (smaller pieces get
# punished), so that one stays.
LATE_GAME_MASS = 8.0
HUNT_MASS_RATIO_LATE = 0.78
MAX_CHASE_ROUNDS_LATE = 45

# --- Stage 5d: fragment merge-cooldown scoring ---
# VisibleBlobModel exposes each blob's merge_cooldown; a fragment with more
# cooldown left stays un-mergeable (and snipeable) longer, so prefer it.
MERGE_COOLDOWN_SCORE_WEIGHT = 0.15

# --- Stage 6a: velocity-led split aim ---
# The child piece SPAWNS ~1.41*our_radius toward the target instantly; only
# the remaining distance is covered by the eject glide (1.6/tick decaying by
# 0.82 -- ~1.5/tick early on). So the actual flight time is short: lead by
# remaining_distance / glide_speed, hard-capped low. (First attempt led by
# total distance with a 12-tick cap: extrapolating a noisy one-tick velocity
# sample that far overshot wildly and lost to v6 8/10 -- the lead must stay
# within the real 2-4 tick flight window.)
SPLIT_LEAD_GLIDE_SPEED = 1.5
SPLIT_LEAD_MAX_TICKS = 4.0
...
def _find_bait_virus(me, my_strongest_mass: float, flee_x: float, flee_y: float, viruses, threats):
    """Stage 5a: a virus worth fleeing TOWARD while we're virus-immune.

    Requirements: the virus can't split us (we're at or under its immunity
    threshold), at least one current threat IS splittable by it, and it lies
    roughly along our flee heading (positive dot product) within baiting
    range -- we bend the escape route, we don't run backwards into danger."""
    best = None
    best_dist = float("inf")
    for virus in viruses:
        virus_mass = _mass(virus.radius)
        if my_strongest_mass > virus_mass * EAT_SIZE_RATIO:
            continue
        if not any(_mass(t.radius) > virus_mass * EAT_SIZE_RATIO for t in threats):
            continue
        to_virus_x, to_virus_y = virus.pos[0] - me.x, virus.pos[1] - me.y
        if to_virus_x * flee_x + to_virus_y * flee_y <= 0:
            continue
        distance = math.hypot(to_virus_x, to_virus_y)
        if distance == 0 or distance > VIRUS_BAIT_MAX_DISTANCE:
            continue
        if distance < best_dist:
            best_dist = distance
            best = virus
...
def _find_fragment_opportunity(me, strongest_mass: float, enemies, size: float, my_radius: float):
    ...
    for blobs in groups.values():
        ...
        for frag in blobs:
            ...
            # Stage 5d: fragments with more merge cooldown left stay
            # snipeable longer, so weight them up.
            score = (
                frag_mass * FRAGMENT_REWARD_MULTIPLIER
                - distance
                + frag.merge_cooldown * MERGE_COOLDOWN_SCORE_WEIGHT
            )
            if score > best_score:
                best_score = score
                best = frag
...
def _is_chase_worth_it(
    my_x: float, my_y: float, target, enemies, my_strongest_radius: float, my_strongest_mass: float,
    max_chase_rounds: float = MAX_CHASE_ROUNDS,
) -> bool:
...
def _split_aim_point(tracker, blob, me_x: float, me_y: float, me_radius: float, size: float):
    """Stage 6a: lead the split shot using the target's MEASURED velocity.

    A split is a one-shot commitment -- the ejected piece can't steer -- and
    _predicted_prey_position only guesses "it flees straight away from us",
    which whiffs against prey moving laterally (e.g. already running
    down-right: we must aim further down-right). Lead only over the true
    flight time: the child spawns ~1.41*our_radius toward the target
    instantly, so only the remaining gap is covered by the glide. Returns
    None when we have no velocity sample yet (first sighting) or no lead is
    needed, so the caller falls back to the naive prediction."""
    last = tracker.last_enemy_positions.get(blob.blob_id)
    if last is None:
        return None
    velocity_x, velocity_y = blob.pos[0] - last[0], blob.pos[1] - last[1]
    if velocity_x == 0.0 and velocity_y == 0.0:
        return None
    distance = math.hypot(blob.pos[0] - me_x, blob.pos[1] - me_y)
    remaining = distance - me_radius * SPLIT_SPAWN_REACH
    if remaining <= 0:
        return None
    lead_ticks = min(remaining / SPLIT_LEAD_GLIDE_SPEED, SPLIT_LEAD_MAX_TICKS)
    predicted_x = blob.pos[0] + velocity_x * lead_ticks
    predicted_y = blob.pos[1] + velocity_y * lead_ticks
    predicted_x = min(max(predicted_x, blob.radius), size - blob.radius)
    predicted_y = min(max(predicted_y, blob.radius), size - blob.radius)
    return (predicted_x, predicted_y)
...
class HuntTracker:
    ...
    onto, a cooldown on targets we've given up on, and (Stage 6a) every
    visible enemy blob's position last tick so we can measure their actual
    velocity instead of guessing their heading."""
    ...
    def __init__(self) -> None:
        ...
        self.last_enemy_positions: dict[int, tuple[float, float]] = {}
...
def choose_direction(game: Game, tracker: HuntTracker) -> tuple[float, float, bool]:
    """Wrapper that refreshes the tracker's last-seen enemy positions AFTER
    the decision is made -- _split_aim_point needs last tick's positions to
    measure velocity, so they must not be overwritten until we're done."""
    result = _choose_direction_impl(game, tracker)
    me = game.state.me
    tracker.last_enemy_positions = {
        blob.blob_id: blob.pos
        for blob in game.state.visible_blobs
        if blob.player_id != me.player_id
    }
    return result


def _choose_direction_impl(game: Game, tracker: HuntTracker) -> tuple[float, float, bool]:
    me = game.state.me
    ...
    if threats or dangerous_viruses:
        # Stage 5b: a threat that can split-lunge onto us gets weighted up
        # while we're inside its burst reach, so the flee buffer is sized for
        # the burst rather than its walking speed.
        danger_items = []
        for blob in threats:
            threat_mass = _mass(blob.radius)
            weight_mass = threat_mass
            if threat_mass >= SPLIT_MIN_MASS and threat_mass / 2.0 > weakest_mass * EAT_SIZE_RATIO:
                lunge_reach = blob.radius * SPLIT_SPAWN_REACH + SPLIT_EJECT_GLIDE
                distance = math.hypot(blob.pos[0] - me.x, blob.pos[1] - me.y)
                if distance < lunge_reach + me.radius:
                    weight_mass = threat_mass * SPLIT_THREAT_WEIGHT_MULT
        ...
        # Stage 5a: while virus-immune, bend the escape route over a virus
        # that lies roughly along the flee heading -- the pursuer either
        # breaks off or gets scattered by it.
        if threats:
            bait = _find_bait_virus(me, strongest_mass, flee_x, flee_y, game.state.visible_viruses, threats)
            if bait is not None:
                to_bait_x, to_bait_y = bait.pos[0] - me.x, bait.pos[1] - me.y
                bait_dist = math.hypot(to_bait_x, to_bait_y)
                flee_magnitude = math.hypot(flee_x, flee_y)
                if bait_dist > 0 and flee_magnitude > 0:
                    flee_x += to_bait_x / bait_dist * flee_magnitude * VIRUS_BAIT_BLEND
                    flee_y += to_bait_y / bait_dist * flee_magnitude * VIRUS_BAIT_BLEND
    ...
    # Stage 5c: once big, food can't sustain us and the safest world is one
    # where nobody else gets big -- hunt closer to the eat limit and commit
    # to longer chases.
    late_game = strongest_mass >= LATE_GAME_MASS
    hunt_ratio = HUNT_MASS_RATIO_LATE if late_game else HUNT_MASS_RATIO
    max_chase_rounds = MAX_CHASE_ROUNDS_LATE if late_game else MAX_CHASE_ROUNDS
    ...
    if fragment_target is not None and _is_chase_worth_it(
        me.x, me.y, fragment_target, enemies, my_strongest_radius, strongest_mass, max_chase_rounds
    ):
        ...
        if should_split:
            # Stage 6a: a split can't steer once fired -- lead it with the
            # target's measured velocity when we have one.
            aim = _split_aim_point(tracker, fragment_target, me.x, me.y, me.radius, size)
            if aim is not None:
                target_x, target_y = aim
    ...
    prey_blobs = [
        blob
        for blob in enemies
        if _mass(blob.radius) < strongest_mass * hunt_ratio
    ...
    if target is not None:
        if _is_chase_worth_it(me.x, me.y, target, enemies, my_strongest_radius, strongest_mass, max_chase_rounds):
            ...
            if should_split:
                # Stage 6a: lead the one-shot split with measured velocity.
                aim = _split_aim_point(tracker, target, me.x, me.y, me.radius, size)
                if aim is not None:
                    target_x, target_y = aim
            return (target_x - me.x, target_y - me.y, should_split)
