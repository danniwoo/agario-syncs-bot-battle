# v10 = v8 with the leaderboard-falsified Stage 7c reverted, plus the
# lessons of an observed top-1 kill pattern (chained multi-split) applied
# both defensively and offensively. v8 kept Stages 7a/7b (adjacent bite,
# no-give-up-when-close -- replay-verified bug fixes) but its 7c (virus
# feeding floor 16 -> 6) REGRESSED live (9.5 avg vs v3's 13.9): feeding
# while small scatters us into pieces that real opponents wipe with the
# multi-split attack our local sparring bots never throw.
#
#   Stage 8a - revert 7c: VIRUS_FEED_MIN_MASS back to 16.
#   Stage 8b - chained-multi-split threat awareness (defense): an enemy's
#     strike range is its walking distance PLUS one eject glide (~8.9) per
#     split it can afford while its pieces still out-mass our weakest blob.
#     Threats inside that reach get 4x repulsion weight; being fragmented
#     shrinks our weakest blob, which extends their affordable chain and
#     automatically makes a scattered us far more skittish.
#   Stage 8c - feast-while-fleeing (offense): a fragmented player whose
#     largest piece we already out-mass, lying roughly along our escape
#     route, bends the flee path over it -- gain mass while escaping.
#   Stage 8d - chained multi-split feast kill (the observed P1 move):
#     against fragmented players, keep splitting toward the pieces while
#     each new half still wins, capped at 8 blobs to keep reserve.
#
# Deliberately deferred: "split to expand vision" (sum of radii grows ~sqrt(n)
# when splitting, and vision scales once the sum exceeds 12) -- mechanically
# real, but splitting on the belief that nothing dangerous is nearby is
# exactly the fragmented-and-blind state the multi-split attack punishes;
# revisit only after 8b proves itself.
#
#   Stage 7a - opportunistic adjacent bite: observed in a replay standing
#     next to an engine-eatable enemy (needs only 1.2x) and walking away,
#     because HUNT_MASS_RATIO=0.7 refuses anything above 0.7x even at
#     touching distance, and the give-up cooldown can fire against a
#     cornered target juking along the wall. Within 2x our radius and a
#     1.25x mass edge: just eat it.
#   Stage 7b - never give up a chase whose target is within 2x our radius
#     (the stalled-distance give-up was firing right before the kill).
#
# --- Inherited from v3 ---
...
# sustainable income sources are kills and viruses.
# Stage 8a REVERTS v8's Stage 7c (floor 16 -> 6): that experiment regressed
# on the live leaderboard (9.5 avg vs v3's 13.9). Feeding while small
# scatters us into pieces tiny enough that real opponents erase us with the
# chained multi-split attack (observed in a replay: a 33.6 consolidated bot
# wiped a 35.2 bot that had just scattered on a virus, halving itself every
# frame toward the pieces). Our local sparring bots don't punish that way,
# which is why local tests missed it. The visible-enemy gate can't cover
# killers arriving from outside our small-size vision either.
VIRUS_FEED_MIN_MASS = 16.0

# --- Stage 7a: opportunistic adjacent bite ---
# Observed in a leaderboard replay: we stood next to an eatable enemy
# (0.87 vs our 1.26 -- the engine only needs a 1.2x ratio) and walked away.
# Two causes: HUNT_MASS_RATIO=0.7 filters out anything between 0.70x and
# 0.83x of our mass even when it's touching us, and the chase give-up logic
# can fire (with a 30-round cooldown) against a cornered target juking along
# the wall. When a clearly-eatable enemy is within bite range, take it --
# no ratio filter, no tracker, no cooldown.
ADJACENT_BITE_RANGE_FACTOR = 2.0  # within our radius * this
ADJACENT_BITE_MASS_RATIO = 1.25  # our strongest mass must exceed target's * this

# --- Stage 7b: never give up an almost-finished chase ---
GIVE_UP_MIN_DISTANCE_FACTOR = 2.0  # no give-up while target is inside our radius * this

# --- Stage 8b: chained-multi-split threat awareness ---
# The P1-vs-P0 replay lesson: a consolidated enemy's real strike range is
...
# chained split adds ~one full eject glide (1.6/(1-0.82) ~= 8.9). Weight
# such threats up while we're inside that reach so a fragmented us starts
# running long before the burst can connect.
CHAIN_SPLIT_GLIDE = 8.9
CHAIN_THREAT_WEIGHT_MULT = 4.0

# --- Stage 8c: feast-while-fleeing ---
...
# piece, steer the flee path over it -- mass gained while escaping, and
# every piece eaten widens our lead over the pursuer.
FEAST_MIN_BLOBS = 4
FEAST_BLEND = 1.2

# --- Stage 8d: chained multi-split feast kill (P1's own move) ---
# Against a fragmented player, keep splitting toward the pieces while each
# new half still out-masses the largest piece; the halvings double our blob
# count every frame, so cap the chain to keep some consolidation in reserve.
...
def _should_chain_split_feast(me, my_strongest_mass: float, target, enemies, viruses) -> bool:
    """Stage 8d: the P1 move -- chained multi-split onto a fragmented player.
...
def _select_prey(tracker: HuntTracker, prey_blobs, me, round_: int):
    ...
    if tracker.target_id is not None and tracker.target_id in by_id:
        ...
        if round_ - tracker.lock_round >= GIVE_UP_WINDOW_ROUNDS:
            # Stage 7b: a target this close is essentially caught -- a
            # cornered prey juking along the wall can stall the distance
            # metric and used to trigger a bogus give-up (with cooldown)
...
def choose_direction(game: Game, tracker: HuntTracker) -> tuple[float, float, bool]:
    ...
    if threats or dangerous_viruses:
        ...
        # get weighted up while we're inside that strike range -- their real
        # reach is several eject glides, not their walking speed, and a
        # fragmented us (tiny weakest blob) extends how many splits they can
        # afford, so this automatically makes us far more skittish while
        # scattered.
        danger_items = []
        for blob in threats:
        ...
        # Stage 8c: if a fragmented player we can feed on lies roughly along
        # the escape route, bend the flee path over it -- mass gained while
        # escaping widens the gap from the pursuer.
        feast_pos = _find_feast(me, strongest_mass, enemies)
        if feast_pos is not None:
            to_feast_x, to_feast_y = feast_pos[0] - me.x, feast_pos[1] - me.y
            if to_feast_x * flee_x + to_feast_y * flee_y > 0:
                feast_dist = math.hypot(to_feast_x, to_feast_y)
                flee_magnitude = math.hypot(flee_x, flee_y)
                if feast_dist > 0 and flee_magnitude > 0:
                    flee_x += to_feast_x / feast_dist * flee_magnitude * FEAST_BLEND
                    flee_y += to_feast_y / feast_dist * flee_magnitude * FEAST_BLEND
    ...
    # Priority 1.5 [Stage 7a]: a clearly-eatable enemy within bite range is
    # free mass RIGHT NOW -- no ratio filter, no tracker lock, no cooldown.
    # (Still respects the corner-deadlock geometry so this can't reintroduce
    # the stuck-at-corner bug.)
    ...
    # too (keeps halving toward the pieces while each half still wins),
    # alongside the original consolidated single lunge.
